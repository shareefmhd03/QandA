...
def edit_tutorials(request, id):
    try:
        if request.session['loggedin']:
            ...
            if request.method == 'POST':
                form = TutorialForm(
                    request.POST, request.FILES, instance=tutorials)
                if form.is_valid():
...
def delete_challenge(request, pk):
    challenge = ChallengeQuestion.objects.get(id=pk)
    challenge.delete()
    return redirect('view_challenges_admin')
...
def admin_dashboard(request):
    ...
    if request.session['loggedin']:
        today_question_count = Question.objects.filter(
            created_at__gte=yesterday, created_at__lte=tomorrow).count()
        today_questions = Question.objects.filter(
            created_at__gte=yesterday, created_at__lte=tomorrow)[:5]
        today_answer_count = Answer.objects.filter(
            created_at__gte=yesterday, created_at__lte=tomorrow).count()
